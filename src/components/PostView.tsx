...
export async function PostView({
  ...
  return (
    <main className="mx-auto w-full max-w-[1152px] px-4 py-10 sm:px-6">
      <article>
        <header className="border-line mb-8 border-b pb-6">
          <p className="mb-3 flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <span className="text-meta text-ink-muted">
              ...
              {categoryPath(locale, post.category, post.subs)} · {post.date}
            </span>
        ...
        <MdxContent source={body} scope={post.id} />
      </article>

      {related.length > 0 && (
        <section aria-label={t('post.relatedRegion')} className="border-line mt-16 border-t pt-8">
          {/* 손으로 이어 둔 위키링크가 먼저고, 모자라면 태그 · 카테고리로 채운다. */}
          <h2 className="text-title-sm text-ink-strong mb-3">{t('post.related')}</h2>
